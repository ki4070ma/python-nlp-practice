...
def q38():
    ...
    for i in set(d.values()):
        tmp_d[i] = list(d.values()).count(i)
    from operator import itemgetter

    new_d = sorted(list(tmp_d.items()), key=itemgetter(1), reverse=True)
    plt.figure(figsize=(10, 60))
    plt.barh(list(range(len(new_d))), [x[1] for x in new_d], align="center")
    plt.yticks(list(range(len(new_d))), [x[0] for x in new_d])
